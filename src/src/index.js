...
function updateInspectorPanel() {
  ...
  inspector.innerHTML = `
      ...
      <div>
        <label style="width:20px; display:inline-block;">Given: </label>
        <select id="given" name="given" value="${given}">
          <option value="0" ${given == 0 ? "selected" : ""}>Surface Area</option>
          <option value="1" ${given == 1 ? "selected" : ""}>Location</option>
        </select>
        <label>Attach To: </label><br>    
        <select id="attachTo" name="given" value="${attachTo}">
         ...
         <label>Attach to all Planes?: </label>   
          <input type="checkbox" ${attachToAll ? "checked" : ""} value = "${attachToAll}" id = "attachToAll">
      </div>
  `;
